...
- 采用无感知的同步机制，用户无需进行任何操作
- 当创建或修改倒计时设置后，延迟 10 分钟将本地数据同步到云端（防抖机制）
- 每次插件通过 Chrome 同步被初始化时自动检查并下载云端数据到本地
- 合理处理数据冲突情况，优先采用时间戳较新的数据
- 云端数据格式与本地存储保持一致，确保数据结构的兼容性
- 考虑 Chrome sync 存储的限制（最大 102,400 字节），合理存储和压缩数据
